...
#ifndef ZSTD_COMPRESSION_STRATEGY_H_
#define ZSTD_COMPRESSION_STRATEGY_H_
...
struct ZstdCompressionStrategy final : public CompressionStrategy {
  ZstdCompressionStrategy(int compression_level = ZSTD_DEFAULT_CLEVEL)
  ...
  inline void DoCompress(buffer_t& buffer, const char* raw, size_t raw_size) {
    ...
    buffer[0] = static_cast<char>(ZSTD_COMPRESSION);
  ...
  static std::string DoDecompress(const std::string& compressed) {
    ...
    size_t dest_size = ZSTD_getDecompressedSize(&compressed.data()[1], compressed.size()-1);
...
#endif  // ZSTD_COMPRESSION_STRATEGY_H_
